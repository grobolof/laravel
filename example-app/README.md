...
⚠️ Сервис `application` монтирует **весь корень репозитория** (`./:${APP_PATH}` в `docker-compose.yml`), а не подкаталог `app`. Laravel (`artisan`, `composer.json`, каталог `app/`) живёт рядом с `docker-compose.yml` и `.env`. `APP_PATH` — путь **внутри контейнера**, а не папка на хосте.
...
1. Если в корне репозитория ещё нет `artisan`, контейнер сам создаст актуальную версию Laravel через `composer create-project`. Файлы репозитория (`docker-compose.yml`, `makefile`, `.env`, `README.md`) не перезаписываются.
2. При первом создании проекта в корневой `.env` (тот же файл, что читает Compose) записываются [`APP_HOST`](#APP_HOST) (как `APP_URL`) и доступы к СУБД из переменных окружения.
...
👇 При первом создании проекта значения уже подставляются в `.env`. Если создаёте проект вручную, замените в `.env` переменные ([DB_CONNECTION](#DB_CONNECTION), [DB_HOST](#DB_HOST), [DB_PORT](#DB_PORT), [DB_DATABASE](#DB_DATABASE), [DB_USERNAME](#DB_USERNAME), [DB_PASSWORD](#DB_PASSWORD)).
...
👇 Если [`MAILPIT_ENABLED`](#MAILPIT_ENABLED)=1, контейнер направит `mail()` PHP в Mailpit и пропишет SMTP в `.env`. Письма смотрите на `http://localhost:8025`.
