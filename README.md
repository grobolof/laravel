...
✊ Образы для быстрого прототипирования актуальной версии [Laravel][1] под соответствующую версию PHP.

## 🚀 Старт

Вы можете перейти в директорию [example-app](example-app), где подробно изложено как правильно и быстро запустить свой проект на Laravel.

Сервис `application` монтирует **корень репозитория** в контейнер (`./:${APP_PATH}` в `docker-compose.yml`), а не подкаталог `app`. Каталог `app/` — это стандартная папка кода Laravel внутри проекта. `APP_PATH` задаёт путь только внутри контейнера.
...
3. **docker-entrypoint.sh** — скрипт выполняется при запуске контейнера. Он содержит логику по установке и настройке
   `Laravel` для работы из коробки. В процессе выполнения скачивается и устанавливается актуальная версия `Laravel` (если еще не установлена) и настраиваются все инструменты для работы. Существующие файлы корня репозитория (`docker-compose.yml`, `.env`, `README.md`) при создании проекта не перезаписываются.
